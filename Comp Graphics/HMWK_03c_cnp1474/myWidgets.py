...
class cl_toolbar :
  ...
  def draw_callback( self ) :
    ...
    self.master.ob_world.create_graphic_objects( self.master.ob_canvas_frame.canvas, self.master.m_ModelData, doClip, doPerspective )

    self.master.statusBar_frame.set( 'Draw callback' )

  def distance_callback( self ) : 
    self.master.dist = tk.simpledialog.askfloat( 'Distance', 'Enter distance', initialvalue = self.master.dist, minvalue = 0 )
    if( self.master.dist == None ) :
      self.master.statusBar_frame.set( "%s", 'Distance Cancelled' )
