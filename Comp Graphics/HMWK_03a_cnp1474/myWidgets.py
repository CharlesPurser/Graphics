...
class cl_toolbar :
  ...
  def load_callback( self ) :
    filename = filedialog.askopenfilename(initialdir="/", title='Select a File', filetypes = [ ( "allfiles", "*" ) ] )
    if( len(filename) == 0 ) :
      self.master.statusBar_frame.set( 'File Canceled' ) 
    else :
      self.master.mesh = ModelData( filename )
  ...
  def draw_callback( self ) :
    if( self.master.mesh == None ):
      ...
      self.master.ob_world.create_graphic_objects( self.master.ob_canvas_frame.canvas, self.master.mesh )
      xmin = v[0]*width
      ymin = v[1]*height
      xmax = v[2]*width
      ymax = v[3]*height
      self.master.ob_canvas_frame.canvas.create_rectangle( width*v[0], height*v[1], width*v[2], height*v[3], outline= "black" )
